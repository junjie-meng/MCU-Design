...
在完成MCU后，我们对LDPC进行了细致的分析，设计并撰写了DFS/BFS/状压dp的C语言及汇编语言代码。为更加方便的实现C语言到机器码的过程，我们结合编译器和Keil 4 开发工具实现了一个Python工具。同时，为提高效率，减少MCU开发成本，我们决定在该Python工具的基础上进行一些改进，在将C语言转为汇编代码的同时，对相关复杂指令进行修改，达到用基础指令实现复杂汇编的目的。基于该Python工具，我们可以将有关C语言代码转为MCU可以运行的汇编代码，辅助完成汇编设计。
